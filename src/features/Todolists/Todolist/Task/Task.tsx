...
const Task = React.memo((props: TaskPropsType) => {
    ...
    const changeTaskTextCallback = useCallback((text: string) => {
        props.changeText(text, props.task.id)
    }, [props])

    const onChangeCheckboxHandler = useCallback((e: ChangeEvent<HTMLInputElement>) => {
        const value = e.currentTarget.checked ? TaskStatusType.Completed : TaskStatusType.New
    ...
    return (
        <li key={props.task.id} className={props.task.status ? "isDone" : ""}
            ...
            />
            <EditableSpan text={props.task.title}
