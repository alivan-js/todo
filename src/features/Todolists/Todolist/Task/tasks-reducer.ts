...
export const changeTaskStatusTC = createAsyncThunk(
  ...
  async (
    param: { todolistID: string; taskID: string; status: number },
    thunkAPI
  ) => {
    thunkAPI.dispatch(setStatus({ status: "loading" }));
    ...
    const currentTask = state.tasks[param.todolistID].find(
      (el) => el.id === param.taskID
    );
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(changeTaskStatusTC.fulfilled, (state, action) => {
        const payload = action.payload;
        if (payload) {
          const index = state[payload.todolistID].findIndex(
