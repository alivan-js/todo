...
const TodoList = React.memo((props: ToDoListProps) => {
    ...
    const filteredTasks = useMemo(() => {
        switch (props.todolist.filter) {
            ...
            case "active" : {
                return tasks[props.todolist.id].filter(el => el.status === TaskStatusType.New)
            }
            case "complete" : {
                return tasks[props.todolist.id].filter(el => el.status === TaskStatusType.Completed)
            }
            default: {
                return tasks[props.todolist.id]
            }
    ...
    return (
        <div style={{width: "275px"}}>
            ...
            <div style={{marginBottom: "10px"}}>
                <AddItemForm addNewElementCallback={addTaskCallback}
